...
namespace OpenEnvironment.App_Logic.DataAccessLayer
{
    ...
    public class db_WQX
    {
        ...
        //added by Mike
        public static Int32? GetWQX_MONLOCIDX_BySite(string Site)
        {
            using (OpenEnvironmentEntities ctx = new OpenEnvironmentEntities())
            {
                try
                {
                    T_WQX_MONLOC val = (from a in ctx.T_WQX_MONLOC
                                        where a.MONLOC_ID == Site
                                        select a).FirstOrDefault();
                    if (val == null) return null;
                    else return val.MONLOC_IDX;
            ...
            global::System.String sAMP_PREP_STORAGE_DESC, global::System.String wQX_SUBMIT_STATUS, Boolean? aCT_IND, Boolean? wQX_IND, String cREATE_USER = "system", string eNTRY_TYPE = "C")
        {
            using (OpenEnvironmentEntities ctx = new OpenEnvironmentEntities())
            {
                ...
                //{
                    ...
                    if (aCT_SUBMEDIA != null && aCT_SUBMEDIA != "") a.ACT_SUBMEDIA = aCT_SUBMEDIA;
                    else a.ACT_SUBMEDIA = null;
                    ...
                    if (rELATIVE_DEPTH_NAME != null && rELATIVE_DEPTH_NAME != "") a.RELATIVE_DEPTH_NAME = rELATIVE_DEPTH_NAME;
                    else a.RELATIVE_DEPTH_NAME = null;
                    if (aCT_DEPTHHEIGHT_MSR != null && aCT_DEPTHHEIGHT_MSR != "") a.ACT_DEPTHHEIGHT_MSR = aCT_DEPTHHEIGHT_MSR;
                    else a.ACT_DEPTHHEIGHT_MSR = null;
                    if (aCT_DEPTHHEIGHT_MSR_UNIT != null && aCT_DEPTHHEIGHT_MSR_UNIT != "") a.ACT_DEPTHHEIGHT_MSR_UNIT = aCT_DEPTHHEIGHT_MSR_UNIT;
                    else a.ACT_DEPTHHEIGHT_MSR_UNIT = null;
                    if (tOP_DEPTHHEIGHT_MSR != null && tOP_DEPTHHEIGHT_MSR != "") a.TOP_DEPTHHEIGHT_MSR = tOP_DEPTHHEIGHT_MSR;
                    else a.TOP_DEPTHHEIGHT_MSR = null;
                    if (tOP_DEPTHHEIGHT_MSR_UNIT != null && tOP_DEPTHHEIGHT_MSR_UNIT != "") a.TOP_DEPTHHEIGHT_MSR_UNIT = tOP_DEPTHHEIGHT_MSR_UNIT;
                    else a.TOP_DEPTHHEIGHT_MSR_UNIT = null;
                    if (bOT_DEPTHHEIGHT_MSR != null && bOT_DEPTHHEIGHT_MSR != "") a.BOT_DEPTHHEIGHT_MSR = bOT_DEPTHHEIGHT_MSR;
                    else a.BOT_DEPTHHEIGHT_MSR = null;
                    if (bOT_DEPTHHEIGHT_MSR_UNIT != null && bOT_DEPTHHEIGHT_MSR_UNIT != "") a.BOT_DEPTHHEIGHT_MSR_UNIT = bOT_DEPTHHEIGHT_MSR_UNIT;
                    else a.BOT_DEPTHHEIGHT_MSR_UNIT = null;
                    if (dEPTH_REF_POINT != null && dEPTH_REF_POINT != "") a.DEPTH_REF_POINT = dEPTH_REF_POINT;
                    else a.DEPTH_REF_POINT = null;
                    if (aCT_COMMENT != null && aCT_COMMENT != "") a.ACT_COMMENT = aCT_COMMENT;
                    else a.ACT_COMMENT = null;
                    if (bIO_ASSEMBLAGE_SAMPLED != null && bIO_ASSEMBLAGE_SAMPLED != "") a.BIO_ASSEMBLAGE_SAMPLED = bIO_ASSEMBLAGE_SAMPLED;
                    else a.BIO_ASSEMBLAGE_SAMPLED = null;
                    if (bIO_DURATION_MSR != null && bIO_DURATION_MSR != "") a.BIO_DURATION_MSR = bIO_DURATION_MSR;
                    else a.BIO_DURATION_MSR = null;
                    if (bIO_DURATION_MSR_UNIT != null && bIO_DURATION_MSR_UNIT != "") a.BIO_DURATION_MSR_UNIT = bIO_DURATION_MSR_UNIT;
                    else a.BIO_DURATION_MSR_UNIT = null;
                    if (bIO_SAMP_COMPONENT != null && bIO_SAMP_COMPONENT != "") a.BIO_SAMP_COMPONENT = bIO_SAMP_COMPONENT;
                    else a.BIO_SAMP_COMPONENT = null;
                    if (bIO_SAMP_COMPONENT_SEQ != null) a.BIO_SAMP_COMPONENT_SEQ = bIO_SAMP_COMPONENT_SEQ;
                    else a.BIO_SAMP_COMPONENT_SEQ = null;
                    if (bIO_REACH_LEN_MSR != null && bIO_REACH_LEN_MSR != "") a.BIO_REACH_LEN_MSR = bIO_REACH_LEN_MSR;
                    else a.BIO_REACH_LEN_MSR = null;
                    if (bIO_REACH_LEN_MSR_UNIT != null && bIO_REACH_LEN_MSR_UNIT != "") a.BIO_REACH_LEN_MSR_UNIT = bIO_REACH_LEN_MSR_UNIT;
                    else a.BIO_REACH_LEN_MSR_UNIT = null;
                    if (bIO_REACH_WID_MSR != null && bIO_REACH_WID_MSR != "") a.BIO_REACH_WID_MSR = bIO_REACH_WID_MSR;
                    else a.BIO_REACH_WID_MSR = null;
                    if (bIO_REACH_WID_MSR_UNIT != null && bIO_REACH_WID_MSR_UNIT != "") a.BIO_REACH_WID_MSR_UNIT = bIO_REACH_WID_MSR_UNIT;
                    else a.BIO_REACH_WID_MSR_UNIT = null;
                    if (bIO_PASS_COUNT != null) a.BIO_PASS_COUNT = bIO_PASS_COUNT;
                    else a.BIO_PASS_COUNT = null;
                    if (bIO_NET_TYPE != null && bIO_NET_TYPE != "") a.BIO_NET_TYPE = bIO_NET_TYPE;
                    else a.BIO_NET_TYPE = null;
                    if (bIO_NET_AREA_MSR != null && bIO_NET_AREA_MSR != "") a.BIO_NET_AREA_MSR = bIO_NET_AREA_MSR;
                    else a.BIO_NET_AREA_MSR = null;
                    if (bIO_NET_AREA_MSR_UNIT != null && bIO_NET_AREA_MSR_UNIT != "") a.BIO_NET_AREA_MSR_UNIT = bIO_NET_AREA_MSR_UNIT;
                    else a.BIO_NET_AREA_MSR_UNIT = null;
                    if (bIO_NET_MESHSIZE_MSR != null && bIO_NET_MESHSIZE_MSR != "") a.BIO_NET_MESHSIZE_MSR = bIO_NET_MESHSIZE_MSR;
                    else a.BIO_NET_MESHSIZE_MSR = null;
                    if (bIO_MESHSIZE_MSR_UNIT != null && bIO_MESHSIZE_MSR_UNIT != "") a.BIO_MESHSIZE_MSR_UNIT = bIO_MESHSIZE_MSR_UNIT;
                    else a.BIO_MESHSIZE_MSR_UNIT = null;
                    if (bIO_BOAT_SPEED_MSR != null && bIO_BOAT_SPEED_MSR != "") a.BIO_BOAT_SPEED_MSR = bIO_BOAT_SPEED_MSR;
                    else a.BIO_BOAT_SPEED_MSR = null;
                    if (bIO_BOAT_SPEED_MSR_UNIT != null && bIO_BOAT_SPEED_MSR_UNIT != "") a.BIO_BOAT_SPEED_MSR_UNIT = bIO_BOAT_SPEED_MSR_UNIT;
                    else a.BIO_BOAT_SPEED_MSR_UNIT = null;
                    if (bIO_CURR_SPEED_MSR != null && bIO_CURR_SPEED_MSR != "") a.BIO_CURR_SPEED_MSR = bIO_CURR_SPEED_MSR;
                    else a.BIO_CURR_SPEED_MSR = null;
                    if (bIO_CURR_SPEED_MSR_UNIT != null && bIO_CURR_SPEED_MSR_UNIT != "") a.BIO_CURR_SPEED_MSR_UNIT = bIO_CURR_SPEED_MSR_UNIT;
                    else a.BIO_CURR_SPEED_MSR_UNIT = null;
                    if (bIO_TOXICITY_TEST_TYPE != null && bIO_TOXICITY_TEST_TYPE != "") a.BIO_TOXICITY_TEST_TYPE = bIO_TOXICITY_TEST_TYPE;
                    else a.BIO_TOXICITY_TEST_TYPE = null;
                    if (sAMP_COLL_METHOD_IDX != null) a.SAMP_COLL_METHOD_IDX = sAMP_COLL_METHOD_IDX;
                    else a.SAMP_COLL_METHOD_IDX = null;
                    if (sAMP_COLL_EQUIP != null && sAMP_COLL_EQUIP != "") a.SAMP_COLL_EQUIP = sAMP_COLL_EQUIP;
                    else a.SAMP_COLL_EQUIP = null;
                    if (sAMP_COLL_EQUIP_COMMENT != null && sAMP_COLL_EQUIP_COMMENT != "") a.SAMP_COLL_EQUIP_COMMENT = sAMP_COLL_EQUIP_COMMENT;
                    else a.SAMP_COLL_EQUIP_COMMENT = null;
                    if (sAMP_PREP_IDX != null) a.SAMP_PREP_IDX = sAMP_PREP_IDX;
                    else a.SAMP_PREP_IDX = null;
                    if (sAMP_PREP_CONT_TYPE != null && sAMP_PREP_CONT_TYPE != "") a.SAMP_PREP_CONT_TYPE = sAMP_PREP_CONT_TYPE;
                    else a.SAMP_PREP_CONT_TYPE = null;
                    if (sAMP_PREP_CONT_COLOR != null && sAMP_PREP_CONT_COLOR != "") a.SAMP_PREP_CONT_COLOR = sAMP_PREP_CONT_COLOR;
                    else a.SAMP_PREP_CONT_COLOR = null;
                    if (sAMP_PREP_CHEM_PRESERV != null && sAMP_PREP_CHEM_PRESERV != "") a.SAMP_PREP_CHEM_PRESERV = sAMP_PREP_CHEM_PRESERV;
                    else a.SAMP_PREP_CHEM_PRESERV = null;
                    if (sAMP_PREP_THERM_PRESERV != null && sAMP_PREP_THERM_PRESERV != "") a.SAMP_PREP_THERM_PRESERV = sAMP_PREP_THERM_PRESERV;
                    else a.SAMP_PREP_THERM_PRESERV = null;
                    if (sAMP_PREP_STORAGE_DESC != null && sAMP_PREP_STORAGE_DESC != "") a.SAMP_PREP_STORAGE_DESC = sAMP_PREP_STORAGE_DESC;
                    else a.SAMP_PREP_STORAGE_DESC = null;
                    if (wQX_SUBMIT_STATUS != null) a.WQX_SUBMIT_STATUS = wQX_SUBMIT_STATUS;
                    if (aCT_IND != null) a.ACT_IND = aCT_IND;
                    if (wQX_IND != null) a.WQX_IND = wQX_IND;
            ...
            String cREATE_USER = "system")
        {
            using (OpenEnvironmentEntities ctx = new OpenEnvironmentEntities())
            {
                ...
                //{
                    ...
                    if (rESULT_DETECT_CONDITION != null)
                    {
                        if (rESULT_DETECT_CONDITION.Length > 34) a.RESULT_DETECT_CONDITION = rESULT_DETECT_CONDITION.Substring(0, 34);
                        else a.RESULT_DETECT_CONDITION = rESULT_DETECT_CONDITION;
                    }
                    ...
                    if (rESULT_VALUE_TYPE != null && rESULT_VALUE_TYPE != "") a.RESULT_VALUE_TYPE = rESULT_VALUE_TYPE;
                    else a.RESULT_VALUE_TYPE = null;
                    if (rESULT_COMMENT != null && rESULT_COMMENT != "") a.RESULT_COMMENT = rESULT_COMMENT;
                    else a.RESULT_COMMENT = null;
                    if (bIO_INTENT_NAME != null) a.BIO_INTENT_NAME = bIO_INTENT_NAME;
                    if (bIO_INDIVIDUAL_ID != null) a.BIO_INDIVIDUAL_ID = bIO_INDIVIDUAL_ID;
                    if (bIO_TAXONOMY != null && bIO_TAXONOMY != "") a.BIO_SUBJECT_TAXONOMY = bIO_TAXONOMY;
                    else a.BIO_SUBJECT_TAXONOMY = null;
                    ...
                    if (fREQ_CLASS_CODE != null && fREQ_CLASS_CODE != "") a.FREQ_CLASS_CODE = fREQ_CLASS_CODE;
                    else a.FREQ_CLASS_CODE = null;
